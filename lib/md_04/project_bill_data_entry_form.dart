...
class _DataEntryFormState extends State<DataEntryForm>
  ...
  Future<void> viewImages() async {
    ...
    String apiURL =
        '${APIHost().apiURL}/project_payment_controller.php/ViewImage';
    PD.pd(text: apiURL);
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          setState(() {
            _imagesLists =
                data.map((url) => ImagesList(emageUrl: url)).toList();
            _isLoading = false;
          });
          ...
          if (_imagesLists.isNotEmpty && mounted) {
            ...
            ScaffoldMessenger.of(_materialSelectionKey.currentContext!)
                .showSnackBar(
              SnackBar(
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'project_bill_data_entry_form.dart');
      if (!mounted) return;
      WaitDialog.hideDialog(context);
      setState(() {
        _isLoading = false;
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          if (data[0]['is_auth'] == 1) {
            ...
            scaffoldMg(
                'This request is currently disabled and cannot be edited.');
            clearFullData();
          } else {
            ...
            setState(() {
              scaffoldMgOk(
                  'request ${data[0]['req_ref_number']} is valid and can have items added');
  ...
  Future<void> _loadUserRequestMaterialList() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({"request_id": _txtRequestId.text}),
      );
  ...
  Future<void> _updateAmount() async {
    double totalAmount =
        double.tryParse(_txtTotalAmount.text.replaceAll(',', '')) ?? 0;
    double unitCost =
        double.tryParse(_txtUnitCost.text.replaceAll(',', '')) ?? 0;
    ...
    if (totalAmount == 0) {
      ...
      if ((totalAmount - expectedAmount).abs() > 0.01) {
        showTopNotification(context,
            'Entered Total Amount (${_txtTotalAmount.text}) does not match Unit Cost x Qty (${NumberStyles.currencyStyle(expectedAmount.toString())})');
      }
    }
  }
  ...
  Future<void> _loadMaterialEstimationCost(
      String? workName, String? costCategory, String? materialName) async {
    try {
      WaitDialog.showWaitDialog(context, message: 'Loading request data');
      String reqUrl =
          '${APIHost().apiURL}/project_payment_controller.php/EstimateMaterialCost';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
      ...
      if (response.statusCode == 200) {
        ...
        PD.pd(text: responseData.toString());
        if (responseData['status'] == 200) {
          if (responseData['estimateData'] is List &&
              responseData['estimateData'].isNotEmpty) {
            ...
            if (responseData['requestdData'] is List &&
              ...
              setState(() {
                _requestedTotalAmount =
                    num.tryParse(requestData['request_amount'].toString())
                            ?.toDouble() ??
                        0.0;
                _totalEstimateAmount = num.tryParse(
                            estimationData['total_estimate_amount'].toString())
                        ?.toDouble() ??
                    0.0;
                _totalEstimateQty = num.tryParse(
                            estimationData['total_estimate_qty'].toString())
                        ?.toDouble() ??
                    0.0;
                _totalRqQty =
                    num.tryParse(requestData['requested_qty'].toString())
                            ?.toDouble() ??
                        0.0;
                _itemDiscounts =
                    num.tryParse(requestData['item_disc'].toString())
                            ?.toDouble() ??
                        0.0;
                int? isEdit =
                    int.tryParse(estimationData['is_edit_allow'].toString());
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text('No request data found.'),
                    backgroundColor: Colors.red),
              );
            }
          } else {
  ...
  Future<void> _addMaterialToList() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/project_payment_controller.php/AddBilledItem';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
      ...
      if (response.statusCode == 200) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(responseData['message'] ??
                      'Billed item insertion successful'),
                  backgroundColor: Colors.green),
            );
            _loadUserRequestMaterialList();
            clearBillItemData();
        ...
        Navigator.pop(context);
        String errorMessage =
            'billing failed with status code ${response.statusCode}';
  ...
  Future<void> _postRequest() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/project_payment_controller.php/PostToApprove';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (status == 200) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(responseData['message'] ?? 'Post to Approved'),
                  backgroundColor: Colors.green),
            );
        ...
        WaitDialog.hideDialog(context);
        String errorMessage =
            'IOU Sending failed with status code ${response.statusCode}';
        if (response.body.isNotEmpty) {
          try {
            final errorData = jsonDecode(response.body);
            errorMessage = errorData['message'] ?? errorMessage;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: ControllerWithAppBar(
          appname: 'Project Payment Request Form(Data entry Form)',
  ...
  List<Widget> _getFormFields() {
    return [
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => InvoiceSummaryDialog(
                  refNumber:
                      RequestNumber.refNumberCon(val: _txtRequestId.text),
                  refId: _txtRequestId.text,
                ),
              ).then((saved) {
                if (saved == true) {}
              });
      ...
      const SizedBox(height: 20),
      buildTextFieldReadOnly(_txtProject, 'Project Name', 'From Project Name',
          Icons.construction_rounded, true, 45),
      const SizedBox(height: 20),
      buildTextFieldReadOnly(_txtProjectLocation, 'Location Name',
          'From Location Name', Icons.store, true, 45),
      const SizedBox(height: 20),
      ...
      const SizedBox(height: 20),
      buildTextFieldReadOnly(_txtReceiverName, 'Receiver Name',
          'Enter receiver name', Icons.person, true, 45),
      const SizedBox(height: 10),
      buildTextFieldReadOnly(_txtBeneficiaryName, 'Beneficiary Name',
          'Enter beneficiary name', Icons.person, true, 45),
      const SizedBox(height: 10),
      buildTextFieldReadOnly(_txtReceiverMobile, 'Receiver Mobile',
          'Enter receiver mobile', Icons.phone, true, 10),
      const SizedBox(height: 10),
      buildTextFieldReadOnly(_txtComment, 'Request Comment',
          'Enter your comment', Icons.comment, true, 200),
      const SizedBox(height: 20),
      ...
      if (_selectedPaymentType == 2)
        Column(
          children: [
            const SizedBox(height: 10),
            buildTextFieldReadOnly(_txtBranch, 'Branch', 'Enter branch',
                Icons.location_on, true, 45),
            const SizedBox(height: 10),
            buildTextFieldReadOnly(_txtAccountNumber, 'Account Number',
                'Enter account number', Icons.account_balance, true, 45),
          ],
        ),
    ];
  }
        ...
        Radio(
          ...
          onChanged: (value) {
            setState(() {
              ...
              if (_selectedPaymentType != 2) {
                _txtBankName.text = _dropdownBank.first;
              }
            });
          },
        ),
          ...
          CustomDropdown(
            ...
            onChanged: (value) {
              ...
              _loadActiveMaterialList(
                  _selectedValueWorkType.toString(), value.toString());
            },
          ),
          const SizedBox(height: 10),
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: _activeRequestList.asMap().entries.map((entry) {
              ...
              double unitAmount =
                  double.tryParse(requestList['actual_amount']) ?? 0;
              ...
              return DataRow(cells: [
                ...
                _buildDataCell(
                    '${NumberStyles.qtyStyle(qty.toString())} ${requestList['uom']}'),
                _buildDataCell(
                    NumberStyles.currencyStyle(reqAmount.toString())),
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      YNDialogCon.ynDialogMessage(context,
                              messageBody:
                                  'Confirm to remove Item ${requestList['material_des']} QTY :${requestList['req_qty']}',
                        ...
                        if (value == 1) {
                          removeRequestItem(
                              context,
                              requestList['idtbl_user_request_list'].toString(),
                              requestList['material_des']);
                        }
                      });
                    },
                  ),
  ...
  Widget _buildPieChart() {
    ...
    double remainingAmount =
        (_totalEstimateAmount - usedAmount).clamp(0, _totalEstimateAmount);
    double exceededAmount = (usedAmount > _totalEstimateAmount)
        ? (usedAmount - _totalEstimateAmount)
        : 0;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            child: PieChart(
              PieChartData(
                ...
                sections: [
                  PieChartSectionData(
                    color: Colors.blue,
                    value: (exceededAmount > 0)
                        ? _totalEstimateAmount
                        : usedAmount,
                    title:
                        '${_getPercentage((exceededAmount > 0) ? _totalEstimateAmount : usedAmount, _totalEstimateAmount)}%',
                  ...
                  if (exceededAmount > 0)
                    PieChartSectionData(
                      ...
                      title:
                          '${_getPercentage(exceededAmount, _totalEstimateAmount)}%',
                  ...
                  if (exceededAmount == 0 && remainingAmount > 0)
                    PieChartSectionData(
                      ...
                      title:
                          '${_getPercentage(remainingAmount, _totalEstimateAmount)}%',
          ...
          Column(
            children: [
              _buildLegendItem(Colors.green, "Estimated", _totalEstimateAmount,
                  _totalEstimateAmount),
              _buildLegendItem(
                  Colors.blue, "Used", usedAmount, _totalEstimateAmount),
              _buildLegendItem(
  ...
  Widget _buildLegendItem(
      Color color, String label, double value, double total) {
    return Row(
      children: [
        Container(
          width: 10,
                    ...
                    children: <Widget>[
                      ...
                      Visibility(
                          ...
                          child: buildDetailRow(
                              'Material ID', _materialId.toString())),
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Visibility(
                          visible: true,
                          child: buildDetailRow(
                              'Qty', _totalEstimateQty.toString())),
                    ],
                  ),
                ),
              ),
