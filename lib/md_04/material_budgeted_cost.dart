...
class MaterialEstimationDialog extends StatefulWidget {
  ...
  @override
  MaterialEstimationDialogState createState() =>
      MaterialEstimationDialogState();
}
...
class MaterialEstimationDialogState extends State<MaterialEstimationDialog> {
  ...
  Future<void> _loadData() async {
    ...
    try {
              ...
              setState(() {
                ...
                _totalRqQty =
                    num.tryParse(requestData['requested_qty'].toString())
                            ?.toDouble() ??
                        0.0;
                _reqDisc = num.tryParse(requestData['item_disc'].toString())
                        ?.toDouble() ??
                    0.0;
              });
            } else {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'material_budgeted_cost.dart');
    ...
    return Column(
      ...
      children: [
        ...
        if (widget.workName != null)
          _buildInfoRow('Work Name:', widget.workName!),
        if (widget.costCategory != null)
          _buildInfoRow('Cost Category:', widget.costCategory!),
        if (widget.materialName != null)
          _buildInfoRow('Material Name:', widget.materialName!),
        const SizedBox(height: 16),
        ...
        _buildNumberRow('Budgeted Amount:', _totalEstimateAmount,
            isCurrency: true),
        _buildNumberRow('Requested Amount:', _requestedTotalAmount,
            isCurrency: true),
        if (_reqDisc > 0) ...[
  ...
  Widget _buildNumberRow(String label, double value,
