...
class OfzPayPointDialogState extends State<OfzPayPointDialog> {
  ...
  Future<void> paymentApprove(BuildContext context, int requestId, int isPaid,
      String comment, String paymentRef) async {
    WaitDialog.showWaitDialog(context, message: 'Processing Request');
    try {
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/PaymentApprove';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
  ...
  Future<void> _paymentReject(BuildContext context, int requestId,
    ...
    try {
      String apiURL =
          '${APIHost().apiURL}/ofz_payment_controller.php/PaymentReject';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
