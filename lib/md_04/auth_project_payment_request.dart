...
void showEstimationDialog(
    BuildContext context, String projectName, String locatioName) async {
  try {
...
class AuthConstructionPaymentRequestScreen extends StatefulWidget {
  ...
  @override
  AuthConstructionPaymentRequestScreenState createState() =>
      AuthConstructionPaymentRequestScreenState();
}

class AuthConstructionPaymentRequestScreenState
    extends State<AuthConstructionPaymentRequestScreen> {
  ...
  Future<void> fetchRequests() async {
    String apiURL =
        '${APIHost().apiURL}/project_payment_controller.php/ListOfRequest';
    PD.pd(text: apiURL);
    ...
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'auth_project_payment_request.dart');
      setState(() {
  ...
  Widget _buildPaymentTypeGroups() {
    ...
    return ListView(
      ...
      children: paymentTypeGroups.entries.map((entry) {
        ...
        final _totalAmount = requests.fold(
            0.0,
            (sum, request) =>
                sum + (double.tryParse(request.totalAmount) ?? 0));
        final _totalVat = requests.fold(
            0.0, (sum, request) => sum + (double.tryParse(request.vat) ?? 0));
        final _totalSSCL = requests.fold(
            0.0, (sum, request) => sum + (double.tryParse(request.sscl) ?? 0));
        final _totalAddDis = requests.fold(0.0,
            (sum, request) => sum + (double.tryParse(request.addDisc) ?? 0));
          ...
          if (request.isAuth == 0 ||
              ...
              (request.isAppro == 0 &&
                  (UserCredentials().AuthCreditLimit! >
                      (double.tryParse(request.totalAmount) ?? 0))))
      ...
      children: [
        ...
        IconButton(
            ...
            onPressed: () async {
              showDialog(
                context: context,
                builder: (context) => EstimationDetailsDialog(
                  locationName: request.locationName.toString(),
