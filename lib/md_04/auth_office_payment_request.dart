...
class AuthOfficePaymentRequest extends StatefulWidget {
  ...
  @override
  AuthOfficePaymentRequestState createState() =>
      AuthOfficePaymentRequestState();
}
...
class AuthOfficePaymentRequestState extends State<AuthOfficePaymentRequest> {
  ...
  Future<void> fetchRequests() async {
    ...
    String apiURL =
        '${APIHost().apiURL}/ofz_payment_controller.php/ListOfRequest';
    PD.pd(text: apiURL);
    try {
      ...
      children: [
        if (request.isAuth == 0 || request.isAuth == -1)
          ElevatedButton(
            ...
            child: Text(request.isAuth == 0 ? "Authorize" : "Rejected",
                style: TextStyle(color: Colors.white)),
          ),
        SizedBox(width: 8),
        if (request.isAppro == 0 &&
            (UserCredentials().AuthCreditLimit! > request.totalAmount))
          ElevatedButton(
  ...
  _Status _getRequestStatus(OfzRequest request) {
    ...
    if (request.isAuth == -1)
      return _Status("Authorization Rejected", Colors.orange);
