...
class PayPointDialogState extends State<PayPointDialog> {
          ...
          setState(() {
            ...
            if (request != null) {
              if (request['is_paid'] != 0) {
                ...
                _txtComment.text =
                    request['pmt_cmt'] == 'na' ? '' : request['pmt_cmt'];
                refNum = request['req_ref_number'] ?? '';
                _txtAmount.text = NumberStyles.currencyStyle(
                    request['total_req_amount'] ?? '0');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          final String message =
              responseData['message'] ?? 'Error loading banks';
          PD.pd(text: message);
        }
      } else {
        PD.pd(text: "HTTP Error: ${response.statusCode}");
            ...
            title: Row(
              children: [
                IconButton(
                  ...
                  icon:
                      Icon(Icons.close, color: CupertinoColors.destructiveRed),
                ),
                Expanded(
                  child: Text(
            ...
            content: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  if (_paymethId == 1) ...[_datePickerField()],
                  _labeledTextField(
                    maxLength: 45,
                    label: 'Comments',
              ...
              CupertinoDialogAction(
                onPressed: () async {
                  final val = await YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Are you sure you want to reject request number ${refNum}?',
                ...
                onPressed: () async {
                  if (_txtPaymentRef.text.isEmpty) {
                    OneBtnDialog.oneButtonDialog(
                      ...
                      message:
                          'Please enter ${_getCommentLabel().toLowerCase()}',
                      btnName: 'OK',
                      icon: Icons.error,
                      iconColor: Colors.red,
                      btnColor: Colors.black,
                  ...
                  final val = await YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Are you sure you want to Confirm Payment request number ${refNum}?',
  ...
  Future<void> _paymentApprove(BuildContext context, int requestId, int isPaid,
      String comment, String paymentRef) async {
    WaitDialog.showWaitDialog(context, message: 'Processing Request');
    try {
      String reqUrl =
          '${APIHost().apiURL}/project_payment_controller.php/PaymentApprove';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
  ...
  Future<void> _paymentReject(BuildContext context, int requestId,
      String comment, String paymentRef) async {
    WaitDialog.showWaitDialog(context, message: 'Processing Request');
    try {
      String apiURL =
          '${APIHost().apiURL}/project_payment_controller.php/PaymentReject';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
