...
class EstimationAuth {
  ...
  factory EstimationAuth.fromJson(Map<String, dynamic> json) {
    return EstimationAuth(
      id: int.tryParse(
              json['idtbl_project_location_estimation_events'].toString()) ??
          0,
...
class EstimationAuthorizedDialog extends StatefulWidget {
  ...
  const EstimationAuthorizedDialog(
      {super.key,
      required this.id,
  ...
  @override
  EstimationAuthorizedDialogState createState() =>
      EstimationAuthorizedDialogState();
}

class EstimationAuthorizedDialogState
    extends State<EstimationAuthorizedDialog> {
        ...
        CupertinoDialogAction(
          onPressed: () async {
            final val = await YNDialogCon.ynDialogMessage(
              context,
              messageBody:
                  'Are you sure you want to Reject the estimation Authorized request number ${widget.id}?',
        ...
        CupertinoDialogAction(
          onPressed: () async {
            final val = await YNDialogCon.ynDialogMessage(
              context,
              messageBody:
                  'Are you sure you want to Authorized request number ${widget.id}?',
              messageTitle: 'Confirm Payment',
              icon: Icons.verified,
              iconColor: Colors.green,
              btnDone: 'Authorized',
  ...
  Future<void> authorizedOrRejectRequest(BuildContext context, int estimationId,
      int estimationReqId, int isApproved) async {
    WaitDialog.showWaitDialog(context, message: 'Processing Request');
    try {
      String apiURL =
          '${APIHost().apiURL}/estimation_controller.php/Authorized';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
