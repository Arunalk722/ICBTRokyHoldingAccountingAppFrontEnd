...
class _IOUSettlementsState extends State<IOUSettlements>
    ...
    for (var iouId in _selectedIOUNumbers) {
      ...
      if (iou != null) {
        ...
        double settledAmount =
            double.tryParse(iou['set_amount'].toString()) ?? 0;
        ...
        if (availableAmount > 0) {
          ...
          distribution.add({
            ...
            'iouNumber':
                IOUNumber.iouNumber(val: iou['idtbl_IOU_list'].toString()),
  ...
  void _settData() {
    if (_txtAmount.text.isEmpty ||
        double.tryParse(_txtAmount.text.replaceAll(',', '')) == null) {
    ...
    if (settlementAmount > totalAvailableAmount) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'Settlement amount (${NumberStyles.currencyStyle(settlementAmount.toString())}) exceeds available amount (${NumberStyles.currencyStyle(totalAvailableAmount.toString())})'),
      ));
      return;
    }
    ...
    for (var dist in distribution) {
      final iouIndex = _iouList.indexWhere((item) =>
          item['idtbl_IOU_list'].toString() == dist['iouId'].toString());

      if (iouIndex != -1) {
        double currentSettled =
            double.tryParse(_iouList[iouIndex]['set_amount'].toString()) ?? 0;
        _iouList[iouIndex]['set_amount'] =
            currentSettled + dist['settledAmount'];
      }
    }

    String iouArray = distribution
        .map((d) =>
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 200) {
          setState(() {
            ...
            _beneficiaryNames = _beneficiaryList
                .map<String>((item) => item['beneficiary_name'].toString())
                .toList();
          });
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content:
                    Text(data['message'] ?? 'Error loading beneficiaries')),
          );
        }
      }
    } catch (e) {
  ...
  Future<void> _loadIOUNumbers(String beneficiaryName) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${APIHost().apiURL}/iou_controller.php/ListIOUNumber'),
        headers: HttpHeader().header,
        body: jsonEncode({
          "request_type":
              _selectedRequestType == 0 ? "office" : "constructions",
                      ...
                      child: ButtonTheme(
                        ...
                        child: DropdownButton<String>(
                          ...
                          items: _iouList.map((iou) {
                            return DropdownMenuItem<String>(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(
                                      'Ref: ${iou['request_ref']} IOU:${IOUNumber.iouNumber(val: iou['idtbl_IOU_list'].toString())}'),
                                  Text(
                                      'Amount: LKR ${NumberStyles.currencyStyle(iou['amount'].toString())} Set: LKR ${NumberStyles.currencyStyle(iou['set_amount'].toString())}',
                          ...
                          onChanged: (String? newValue) {
                            if (newValue != null &&
                                !_selectedIOUNumbers.contains(newValue)) {
                              setState(() {
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                      'IOU:${IOUNumber.iouNumber(val: iou['idtbl_IOU_list'].toString())}'),
                                  Text('Ref: ${iou['request_ref']}'),
                                  Text(
                                      'Amount: LKR ${NumberStyles.currencyStyle(iou['amount'].toString())}',
                                      style: const TextStyle(fontSize: 12)),
                                  Text(
                                      'Amount: LKR ${NumberStyles.currencyStyle(iou['set_amount'].toString())}',
                                      style: const TextStyle(fontSize: 12)),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            buildTextField(_txtDes, 'References/Description', 'Description',
                Icons.room_preferences, true, 100),
            TextFormField(
            ...
            buildNumberField(
                _txtAmount, 'Settlement Amount', '2000', LKRIcon(), true, 20),
            ElevatedButton(
              onPressed: () {
                _settData();
              },
    ...
    return ConstrainedBox(
      ...
      child: SingleChildScrollView(
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            border:
                TableBorder.all(width: 1, color: Colors.grey.withOpacity(0.2)),
            columnSpacing: 12,
            dataRowMinHeight: 50,
            dataRowMaxHeight: 60,
            headingRowHeight: 50,
                      ...
                      child: IconButton(
                        ...
                        onPressed: () {
                          _showDeleteConfirmation(
                              context, settlement['originalSettlement']);
                        },
                      ),
                    ),
                  ),
      ...
      if (value == 1) {
        setState(() {
          for (var dist in settlement.distribution) {
            ...
            if (iouIndex != -1) {
              double currentSettled = double.tryParse(
                      _iouList[iouIndex]['set_amount'].toString()) ??
                  0;
              _iouList[iouIndex]['set_amount'] =
                  currentSettled - dist['settledAmount'];
            }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 200) {
          OneBtnDialog.oneButtonDialog(context,
              title: 'Process done',
              message:
                  '${data['message']} process number :${data['process_id']}',
              btnName: 'Ok',
              icon: Icons.verified,
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(data['message'] ?? 'Error saving settlements')),
          );
        }
      } else {
        Navigator.pop(context);
