...
class _ProjectBillRequestState extends State<ProjectBillRequest> {
  ...
  Future<void> _uploadImage(String endPoint, int requestId) async {
    ...
    try {
      ...
      String apiURL =
          "${APIHost().apiURL}/project_payment_controller.php/ImageUpload";
      PD.pd(text: apiURL);
      ...
      request.headers['x-token'] = "Bearer ${APIToken().token}";
      ...
      if (response.statusCode == 200 && decodedResponse['status'] == 200) {
        ...
        OneBtnDialog.oneButtonDialog(context,
            ...
            message:
                'Upload failed: ${decodedResponse['message'] ?? 'Unexpected response format.'}',
            btnName: 'Retry',
            icon: Icons.error,
            iconColor: Colors.red,
            btnColor: Colors.red);
  ...
  Future<void> getProjectSummary(String projectName, String location) async {
    try {
      ...
      String apiURL =
          '${APIHost().apiURL}/project_controller.php/GetProjectSummary';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
  ...
  Future<void> createNewRequest() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/project_payment_controller.php/InsertPaymentRequest';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
      ...
      if (response.statusCode == 200) {
        ...
        if (status == 200) {
          requestId = int.tryParse(
                  responseData['tbl_user_payment_request_id'].toString()) ??
              0;
          _txtRequestId.text = RequestNumber.formatNumber(val: requestId);
          _btnVisible = false;
          YNDialogCon.ynDialogMessage(context,
                  messageTitle: 'Request Creating successful',
                  messageBody:
                      '${responseData['message']} would you like to upload bill',
                  icon: Icons.verified,
                  iconColor: Colors.green,
                  btnDone: 'Upload',
                  btnClose: 'Later')
            ...
            if (value == 1) {
              if (kIsWeb) {
                await startFilePicker(
                    int.tryParse(requestId.toString()) ?? 0,
                    RequestNumber.refNumberCon(
                        val: _txtRequestId.text.toString()));
              } else {
                WaitDialog.hideDialog(context);
              }
            } else {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          await startFilePicker(int.tryParse(requestId.toString()) ?? 0,
              RequestNumber.refNumberCon(val: _txtRequestId.text.toString()));
        },
        backgroundColor: Colors.green,
    ...
    return [
      ...
      CustomDropdown(
        ...
        onChanged: (value) {
          ...
          getProjectSummary(
              _txtProjectDropdown.text, _txtProjectLocationDropdown.text);
        },
      ),
      const SizedBox(height: 20),
      ...
      buildTextField(_txtComment, 'Request Comment', 'Enter your comment',
          Icons.comment, true, 200),
      const SizedBox(height: 10),
      buildNumberField(_txtRequestedAmount, 'Request Amount',
          'Enter expected amount', LKRIcon(), true, 10),
      Column(
        children: [
          DatePickerWidget(
            label: 'Expected date',
              ...
              child: Column(
                ...
                children: [
                  ...
                  SummaryRow(
                    ...
                    label: 'Estimated',
                    value:
                        'Rs. ${NumberStyles.currencyStyle(data['total_estimation_amount'])}',
                  ),
                  SummaryRow(
                    icon: Icons.receipt_long_outlined,
                    label: 'Spent',
                    value:
                        'Rs. ${NumberStyles.currencyStyle(data['total_actual_amount'])}',
                  ),
                  const SizedBox(height: 20),
                  SizedBox(
