...
class ExpAndTaskDesignerState extends State<ExpAndTaskDesigner> {
  ...
  Future<void> _createWorkType() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/material_controller.php/CreateWorkList';
      PD.pd(text: reqUrl);
      ...
      if (response.statusCode == 200) {
        try {
          ...
          ExceptionLogger.logToError(
              ...
              logFile: 'exp_and_task_designer.dart');
  ...
  Future<void> _createCostCategory() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/material_controller.php/CreateCostCategory';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
              ...
              if (_txtWorkType.text.length > 1) {
                if (!workTypes.contains(_txtWorkType.text)) {
                  YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Confirm to create Work Type ${_txtWorkType.text}',
  ...
  Widget _buildCategorySection() {
    return Column(
      ...
      children: [
        ...
        Autocomplete<String>(
          initialValue:
              TextEditingValue(text: _selectedValueCostCategory ?? ''),
          optionsBuilder: (textEditingValue) {
            final input = textEditingValue.text.toLowerCase();
              ...
              if (_selectedWorkType == null) {
                ...
                if (!categories.contains(_txtCostCategory.text)) {
                  YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Confirm to create Cost Category ${_txtCostCategory.text} under Work Type $_selectedWorkType',
