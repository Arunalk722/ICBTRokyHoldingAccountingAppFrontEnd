...
class _ViewIOUScreenState extends State<ViewIOUScreen> {
  ...
  Widget buildProjectGroupedTable() {
    ...
    for (var iou in _iouList) {
      var groupKey =
          "${iou['project_name'] ?? ''} - ${iou['location_name'] ?? ''}";
      ...
      projectTotals.update(groupKey, (value) => value + amount,
          ifAbsent: () => amount);
    ...
    var sortedGroupKeys = groupedIOUs.keys.toList()
      ..sort((a, b) {
        int idA =
            int.tryParse(groupedIOUs[a]!.first['iou_id']?.toString() ?? '0') ??
                0;
        int idB =
            int.tryParse(groupedIOUs[b]!.first['iou_id']?.toString() ?? '0') ??
                0;
        return idA.compareTo(idB);
      });
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Text(
                            groupKey == ' - '
                                ? 'Office IOU'
                                : "Project: ${groupKey.split(' - ')[0]}\nLocation: ${groupKey.split(' - ')[1]}",
                            style: TextStyle(
                                color: Colors.blue[800],
                                fontWeight: FontWeight.bold),
                          ),
                      ...
                      TableRow(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Text(
                                DateFormat('yyyy-MM-dd').format(
                                    DateTime.parse(iou['iou_created_date'])),
                          ...
                          Padding(
                            padding: const EdgeInsets.all(12.0),
                            child: Text(
                                iou['request_type']?.toString().toUpperCase() ??
                                    'N/A',
                                style: TextStyle(color: Colors.grey[800])),
                          ),
                          Padding(
  ...
  Widget buildDateGroupedTable() {
    ...
    for (var iou in _iouList) {
      ...
      double amount = double.tryParse(iou['amount']?.toString() ?? '0') ?? 0.0;
      dateTotals.update(formattedDate, (value) => value + amount,
          ifAbsent: () => amount);
    ...
    var sortedDates = groupedIOUs.keys.toList()..sort((a, b) => a.compareTo(b));
                      ...
                      for (var iou in groupedIOUs[date]!)
                        TableRow(
                          ...
                          children: [
                            Padding(
                              padding: const EdgeInsets.all(12.0),
                              child: Text(iou['project_name'] ?? 'Office',
                                  style: TextStyle(color: Colors.grey[800])),
                            ),
                            Padding(
                              padding: const EdgeInsets.all(12.0),
    ...
    try {
      ...
      for (var iou in iouList) {
        ...
        rows.add([
          ...
          paymentRequestId,
          iou['receiver_name']?.toString() ??
              iou['OfzReceiver']?.toString() ??
              'N/A',
          beneficiaryName,
          accountNumber,
          paymentRef,
