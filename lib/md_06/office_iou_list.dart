...
class OfficeIOUList extends StatefulWidget {
  ...
  @override
  OfficeIOUListState createState() => OfficeIOUListState();
}

class OfficeIOUListState extends State<OfficeIOUList> {
  ...
  Future<void> fetchRequests() async {
    ...
    String apiURL =
        '${APIHost().apiURL}/ofz_payment_controller.php/ViewOfficeIOUReport';
  ...
  Widget buildTable() {
    _requests.sort((a, b) => DateTime.parse(a.payedDate.toString())
        .compareTo(DateTime.parse(b.payedDate.toString())));
    ...
    for (var iou in _requests) {
      var groupKey = DateFormat('yyyy-MM-dd')
          .format(DateTime.parse(iou.payedDate.toString()));
      ...
      projectTotals.update(groupKey, (val) => val + amount,
          ifAbsent: () => amount);
                    ...
                    for (var iou in groupedIOUs[groupKey]!)
                      TableRow(
                        decoration: BoxDecoration(
                          color: groupedIOUs[groupKey]!.indexOf(iou) % 2 == 0
                              ? Colors.white
                              : Colors.grey[50],
                        ),
                        children: [
                          ...
                          Padding(
                            padding: EdgeInsets.all(12.0),
                            child: Text(
                                IOUNumber.iouNumber(val: iou.id.toString()),
                                style: TextStyle(color: Colors.grey[800])),
                          ),
                          Padding(
                            padding: EdgeInsets.all(12.0),
                            child: Text(iou.refNum,
                                style: TextStyle(color: Colors.grey[800])),
                          ),
                          Padding(
                            padding: EdgeInsets.all(12.0),
                          ...
                          Padding(
                            ...
                            child: Text(
                              NumberStyles.currencyStyle(
                                  ((double.tryParse(iou.totalAmount) ?? 0.0) +
                                          (double.tryParse(iou.vat) ?? 0.0) +
                                          (double.tryParse(iou.sscl) ?? 0.0) -
                                          (double.tryParse(iou.addDis) ?? 0.0) -
                                          (double.tryParse(iou.itemDis) ?? 0.0))
                                      .toStringAsFixed(2)),
                              style: TextStyle(color: Colors.grey[800]),
                              textAlign: TextAlign.right,
                            ),
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.pop(context);
                  exportPaymentRequestsToCSV(parentContext, _requests);
                },
              ),
            ],
          ),
  ...
  Future<void> exportPaymentRequestsToCSV(
      BuildContext context, List<PaymentRequest> requests) async {
    try {
