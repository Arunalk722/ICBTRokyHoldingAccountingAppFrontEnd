...
class _OfficePaymentRequestState extends State<OfficePaymentRequest>
  ...
  Future<void> _loadUserRequestMaterialList() async {
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({"ofz_request_id": _txtRequestId.text}),
      );
  ...
  Future<void> _loadTemplateList() async {
    try {
      ...
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({"created_by": UserCredentials().UserName}),
      );
  ...
  Future<void> _loadTemplateData() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/payment_temp_controller.php/SelectTemplate';
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({
        ...
        if (responseData['status'] == 200) {
          ...
          if (responseData['data'] is List && responseData['data'].isNotEmpty) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('No payment template data found.'),
  ...
  Future<void> createPaymentTemplate() async {
    try {
      String url =
          '${APIHost().apiURL}/payment_temp_controller.php/RegisterPaymentTemplate';
      final response = await http.post(
        Uri.parse(url),
        headers: HttpHeader().header,
        body: jsonEncode({
          ...
          "bank_name":
              _selectedPaymentType == 2 ? _selectedValueBank.toString() : "NO",
          "rec_name": _txtReceiverName.text,
      ...
      if (response.statusCode == 200) {
        ...
        String errorMessage =
            'Payment Template creation failed with status code ${response.statusCode}';
  ...
  Future<void> deleteProfile(String recName) async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/payment_temp_controller.php/DeleteProfile';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode(
            {"created_by": UserCredentials().UserName, "rec_name": recName}),
      );
      if (response.statusCode == 200) {
        Navigator.pop(context);
        final Map<String, dynamic> responseData = jsonDecode(response.body);
        ...
        Navigator.pop(context);
        String errorMessage =
            'Request creation failed with status code ${response.statusCode}';
        if (response.body.isNotEmpty) {
          try {
            final errorData = jsonDecode(response.body);
            errorMessage = errorData['message'] ?? errorMessage;
      ...
      ExceptionDialog.exceptionDialog(context,
          title: 'Error',
          message: errorMessage,
          btnName: 'Ok',
          icon: Icons.sms_failed,
  ...
  Future<void> _loadOfzSubCategoryList(String? mainCategory) async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/GetSubCategory';
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({
  ...
  Future<void> _loadOfzItemList(String? mainCategory, String? subName) async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/GetMaterialItem';
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({
  ...
  Future<void> _dropDownToProject() async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/project_controller.php/ProjectByDate';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
  ...
  Future<void> _dropDownToProjectLocation(String project) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({"project_name": project}),
      );
  ...
  Future<void> createNewOfzRequest() async {
    try {
      if (_selectedPaymentType == 2 &&
          (_selectedValueBank == null || _selectedValueBank!.isEmpty)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      String url =
          '${APIHost().apiURL}/ofz_payment_controller.php/CreateOfzPaymentRequest';
      final response = await http.post(
        Uri.parse(url),
        headers: HttpHeader().header,
        body: jsonEncode({
          ...
          "account_number":
              _selectedPaymentType == 2 ? _txtAccountNumber.text : 'No',
          ...
          "sscl":
              _txtSSCL.text.isEmpty ? '0' : _txtSSCL.text.replaceAll(',', ''),
          "add_dis": _txtAddiDis.text.isEmpty
              ? '0'
              : _txtAddiDis.text.replaceAll(',', ''),
          "is_active": 0,
          "created_by": UserCredentials().UserName,
        }),
      );
      if (response.statusCode == 200) {
        ...
        if (status == 200) {
          ...
          requestId =
              int.tryParse(responseData['tbl_ofz_request_id'].toString()) ?? 0;
  ...
  Future<void> _addItemsToRequest() async {
    try {
      if (_selectedOfzMainCategoryItem == null ||
          _selectedOfzMainCategoryItem!.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      if (_selectedOfzSubCategoryItem == null ||
          _selectedOfzSubCategoryItem!.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      String url =
          '${APIHost().apiURL}/ofz_payment_controller.php/InsertRequestList';
      final response = await http.post(
        Uri.parse(url),
        headers: HttpHeader().header,
        body: jsonEncode({
          ...
          "created_by": UserCredentials().UserName,
          'uom': _txtUnit.text,
          'item_dis': _txtItemDisc.text.isEmpty
              ? '0'
              : _txtItemDisc.text.replaceAll(',', ''),
  ...
  Future<void> _postRequest() async {
    try {
      ...
      String url =
          '${APIHost().apiURL}/ofz_payment_controller.php/PostToApprove';
      final response = await http.post(
        Uri.parse(url),
        headers: HttpHeader().header,
        body: jsonEncode({
  ...
  void _totalCost() {
    double totalAmount =
        double.tryParse(_txtCostAmount.text.replaceAll(',', '')) ?? 0;
    double unitCost =
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: ControllerWithAppBar(
          appname: 'Office Payment Request Form',
  ...
  List<Widget> _getFormFields() {
    return [
      ...
      const SizedBox(height: 10),
      buildTextField(_txtBeneficiaryName, 'Beneficiary Name',
          'Enter beneficiary name', Icons.person, true, 45),
      const SizedBox(height: 10),
      buildTextField(_txtReceiverMobile, 'Receiver Mobile',
          'Enter receiver mobile', Icons.phone, true, 10),
      const SizedBox(height: 10),
      buildTextField(_txtComment, 'Request Comment', 'Enter your comment',
          Icons.comment, true, 45),
      _buildTax(context),
      Column(
        children: [
          DatePickerWidget(
      ...
      if (_selectedPaymentType == 2)
        Column(
          children: [
            const SizedBox(height: 10),
            buildTextField(_txtBranch, 'Branch', 'Enter branch',
                Icons.location_on, true, 45),
            const SizedBox(height: 10),
            buildTextField(_txtAccountNumber, 'Account Number',
      ...
      builder: (context, constraints) {
        if (constraints.maxWidth < 600) {
          return Column(
            children: [
              buildNumberField(
                  _txtVat, 'VAT', 'Total VAT', LKRIcon(), true, 20),
              SizedBox(height: 10),
              buildNumberField(
                  _txtSSCL, 'SSCL', 'Total SSCL', LKRIcon(), true, 20),
              SizedBox(height: 10),
              buildNumberField(_txtAddiDis, 'Additional discount',
                  'Total Discount', LKRIcon(), true, 20),
            ],
          );
        } else {
          return Column(
    ...
    Widget buildFormFields() {
      return Column(
        children: [
              ...
              Expanded(
                ...
                child: CustomDropdown(
                  ...
                  onChanged: (value) {
                    ...
                    _loadOfzItemList(
                        _txtMaterialDropDown.text, value.toString());
          ...
          Row(
            children: [
              Expanded(flex: 6, child: _buildMaterialSection()),
              Expanded(
                flex: 3,
                child: Container(
          ...
          buildTextField(
              _txtItemDes,
              'Descriptions',
              _selectedOfzMainCategoryItem == 'Vehicle'
                  ? 'Petrol'
                  : _selectedOfzMainCategoryItem == 'Food'
                      ? 'Lunch/Dinner'
                      : "Books",
              Icons.description,
              true,
      ...
      children: [
        ...
        Autocomplete<String>(
          ...
          optionsBuilder: (TextEditingValue textEditingValue) {
            return _dropdownOfzItem.where((word) => word
                .toLowerCase()
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            if (_activeRequestList.isNotEmpty) _buildCloseButton(),
          ],
        ),
      ),
    );
                    ...
                    Padding(
                      ...
                      child: DataTable(
                        ...
                        rows: _activeRequestList.map((requestList) {
                          double total =
                              double.tryParse(requestList['total_amout']) ?? 0;
                          double unitAmount =
                              double.tryParse(requestList['amout']) ?? 0;

                          String formattedTotal =
                              NumberFormat('#,###.00', 'en_US').format(total);
                          ...
                          return DataRow(
                            cells: [
                              _buildDataCell(
                                  requestList['main_name'].toString()),
                              _buildDataCell(
                                  requestList['sub_name'].toString()),
                              _buildDataCell(
                                  requestList['item_name'].toString()),
                              _buildDataCell(
                                  requestList['list_des'].toString()),
                              _buildDataCell(requestList['ref'].toString()),
                              _buildDataCell(
                                  '${requestList['qty']} ${requestList['uom']}'),
                              ...
                              DataCell(
                                IconButton(
                                  icon: const Icon(Icons.delete_forever_rounded,
                                      color: Colors.red),
                                  onPressed: () =>
                                      _confirmDeleteItem(requestList),
                                ),
                              ),
                            ],
                          );
  ...
  void _confirmDeleteItem(Map<String, dynamic> requestList) {
    YNDialogCon.ynDialogMessage(
      context,
      messageBody:
          'Confirm to remove Item ${requestList['list_des']} QTY: ${requestList['qty']}',
    ...
    return Column(
      ...
      children: [
        ...
        Autocomplete<String>(
          ...
          optionsBuilder: (TextEditingValue textEditingValue) {
            return _dropdownReceiver.where((word) => word
                .toLowerCase()
                .contains(textEditingValue.text.toLowerCase()));
          },
          onSelected: (String selection) {
                            ...
                            GestureDetector(
                              onTap: () async {
                                int a = await YNDialogCon.ynDialogMessage(
                                  context,
                                  messageBody:
                                      'Remove selected payment profile',
  ...
  Future<void> removeRequestItem(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(
            '${APIHost().apiURL}/ofz_payment_controller.php/RemoveItemFormList'),
        headers: HttpHeader().header,
        body: jsonEncode({
