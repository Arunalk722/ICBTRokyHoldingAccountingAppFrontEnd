...
class RequestPaymentProcessState extends State<RequestPaymentProcess> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'payment_process_all.dart');
      setState(() {
  ...
  Future<List<PaymentRequest>> fetchRequests() async {
    String apiURL =
        '${APIHost().apiURL}/project_payment_controller.php/ViewNotPayedRequests';
    PD.pd(text: apiURL);
  ...
  Future<List<PaymentRequest>> fetchRequestsOfzPayment() async {
    String apiURL =
        '${APIHost().apiURL}/ofz_payment_controller.php/ViewNotPayedRequests';
    PD.pd(text: apiURL);
    final response = await http.get(
      Uri.parse(apiURL),
      headers: HttpHeader().header,
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TabBarView(
                          children: [
                            _buildRequestList(
                                categorizedRequests["Cash"] ?? [], screenWidth),
                            _buildRequestList(
  ...
  Widget _buildRequestList(List<PaymentRequest> requests, double screenWidth) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        double _billAmount =
            double.tryParse(request.totalActualAmount.toString()) ?? 0;
              ...
              title: Row(
                children: [
                  ...
                  Expanded(
                      child: StatusIcon(
                          statusId:
                              int.tryParse(request.statusId.toString()) ?? 0))
                ],
              ),
              children: [
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (request.projectName != 'NA') ...[
                        _infoText("Project", request.projectName.toString(),
                            Colors.black),
                        _infoText("Location", request.locationName.toString(),
                            Colors.black),
                      ],
                      _infoText(
                          "Request Number",
                          RequestNumber.formatNumber(
                              val: int.tryParse(request.id.toString()) ?? 0),
                          Colors.black),
                      _infoText("Receiver", request.receiverName.toString(),
                          Colors.black),
                      _infoText("Mobile", request.receiverMobile.toString(),
                          Colors.grey.shade700),
                      _infoText("Date", request.requestDate.toString(),
                          Colors.grey.shade700),
                      _infoText("Comment", request.comment.toString(),
                          Colors.grey.shade700),
                      _infoText(
                        "Total Amount",
                      ...
                      if (request.paymentType == "Bank Transfer") ...[
                        ...
                        _infoText(
                            "Beneficiary",
                            request.beneficiaryName.toString(),
                            Colors.grey.shade700),
                        _infoText("Account Number",
                            request.accountNumber.toString(), Colors.black),
                        _infoText("Bank Name", request.bankName.toString(),
                            Colors.black),
                        _infoText("Bank Branch", request.bankBranch.toString(),
                            Colors.black),
                      ],
                      if (request.isAuth.toString() == '1') ...[
                        SizedBox(height: 8),
                        _infoText("Authorized by", request.authUser ?? 'N/A',
                            Colors.blue.shade700),
                        _infoText("Auth Comment", request.authCmt ?? 'N/A',
                            Colors.blue.shade700),
                        _infoText("Auth Time", request.authTime ?? 'N/A',
                            Colors.blue.shade700),
                      ],
                      if (request.isAppro.toString() == '1') ...[
                        SizedBox(height: 8),
                        _infoText("Approved by", request.approUser.toString(),
                            Colors.green.shade700),
                        _infoText("Approval Comment",
                            request.approCmt.toString(), Colors.green.shade700),
                        _infoText("Approval Time", request.approTime.toString(),
                            Colors.green.shade700),
                      ],
