...
class _ProjectDashboardState extends State<ProjectDashboard> {
  ...
  Future<void> _fetchDashboardData() async {
    ...
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/report_controller.php/ViewProjectDashBoard';
      PD.pd(text: reqUrl);
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          final String message =
              responseData['message'] ?? 'Error fetching data';
          PD.pd(text: message);
  ...
  Widget _buildSummaryCards() {
    ...
    double totalRequested = _projectData.fold(
        0,
        (sum, item) =>
            sum +
    ...
    double totalEstimated = _projectData.fold(
        0,
        (sum, item) =>
            sum + (double.tryParse(item['estimated_amount'].toString()) ?? 0));
          ...
          return Wrap(
            ...
            children: [
              ...
              _buildStatCard(
                title: 'Variance',
                value:
                    '${NumberStyles.currencyStyle(variance.toString())} (${variancePercentage.toStringAsFixed(1)}%)',
                icon: Icons.compare,
  ...
  Widget _buildProjectDistributionChart() {
    ...
    for (var i = 10; i < sortedProjects.length; i++) {
      othersTotal += (double.tryParse(
                  sortedProjects[i]['totalReq'].toString()) ??
              0) -
          (double.tryParse(sortedProjects[i]['item_disc'].toString()) ?? 0) +
          (double.tryParse(
                  sortedProjects[i]['ofz_list_total_amout'].toString()) ??
              0) -
          (double.tryParse(sortedProjects[i]['ofz_list_item_dis'].toString()) ??
              0);
    }
    ...
    for (var i = 0; i < top10Projects.length; i++) {
      ...
      final percentage =
          totalRequested > 0 ? (amount / totalRequested * 100) : 0;
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    child: Column(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...top10Projects.asMap().entries.map((entry) {
                          final index = entry.key;
                          final data = entry.value;
                          final amount = (double.tryParse(
                                      data['totalReq'].toString()) ??
                                  0) -
                              ...
                              (double.tryParse(data['ofz_list_total_amout']
                                  ...
                                  0) -
                              (double.tryParse(
                                      data['ofz_list_item_dis'].toString()) ??
                                  0);
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Text(
                                  '${(othersTotal / totalRequested * 100).toStringAsFixed(1)}%',
                                  style: const TextStyle(fontSize: 12),
                                ),
                              ],
                            ),
  ...
  Widget _buildComparisonChart() {
    ...
    for (final data in _projectData) {
      ...
      final totalReq =
          double.tryParse(data['totalReq']?.toString() ?? '0') ?? 0;
      final ofzAmount =
          double.tryParse(data['ofz_list_total_amout']?.toString() ?? '0') ?? 0;
      final ofzDisc =
          double.tryParse(data['ofz_list_item_dis']?.toString() ?? '0') ?? 0;
      final itemDisc =
          double.tryParse(data['item_disc']?.toString() ?? '0') ?? 0;
      final addDisc =
          double.tryParse(data['addt_discount']?.toString() ?? '0') ?? 0;

      final requested =
          totalReq + sscl + vat + ofzAmount - ofzDisc - itemDisc - addDisc;
      final estimated =
          double.tryParse(data['estimated_amount']?.toString() ?? '0') ?? 0;
              ...
              child: SfCartesianChart(
                ...
                tooltipBehavior: TooltipBehavior(
                  ...
                  builder: (data, point, series, pointIndex, seriesIndex) {
                    final location =
                        chartData[pointIndex]['location'] as String;
                    final requested =
                        chartData[pointIndex]['requested'] as double;
                    final estimated =
                        chartData[pointIndex]['estimated'] as double;
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                              'Requested: ${NumberFormat.currency(symbol: 'Rs. ').format(requested)}'),
                          Text(
                              'Estimated: ${NumberFormat.currency(symbol: 'Rs. ').format(estimated)}'),
                          const SizedBox(height: 4),
                          Text(
                            'Variance: ${NumberFormat.currency(symbol: 'Rs. ').format(variance)} (${variancePercent.toStringAsFixed(1)}%)',
                            style: TextStyle(
            ...
            SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: _projectData.map((data) {
                  double requested = (double.tryParse(
                              data['sscl'].toString()) ??
                          0) +
                      (double.tryParse(data['vat'].toString()) ?? 0) +
                      (double.tryParse(data['totalReq'].toString()) ?? 0) +
                      (double.tryParse(
                              data['ofz_list_total_amout'].toString()) ??
                          0) -
                      (double.tryParse(data['ofz_list_item_dis'].toString()) ??
                          0) -
                      (double.tryParse(data['item_disc'].toString()) ?? 0) -
                      (double.tryParse(data['addt_discount'].toString()) ?? 0);
                  double estimated =
