...
class RemindersScreenState extends State<RemindersScreen> {
  ...
  Future<void> _loadReminders() async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            '${APIHost().apiURL}/reminders_controller.php/GetRemindersByUser'),
        headers: HttpHeader().header,
        body: jsonEncode({"create_by": UserCredentials().UserName}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 200) {
          ...
          showErrorDialog(
              context, data['message'] ?? 'Failed to load reminders');
        }
      } else {
  ...
  void _confirmDeleteReminder(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete "${reminder['reminder_log']}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              await _deleteReminder(
                  context, reminder['idtbl_my_reminders'].toString());
            },
  ...
  Future<void> _createReminder(
      BuildContext context, String text, String date) async {
    try {
      WaitDialog.showWaitDialog(context, message: 'Creating reminder...');
      final response = await http.post(
        Uri.parse(
            '${APIHost().apiURL}/reminders_controller.php/CreateReminder'),
        headers: HttpHeader().header,
        body: jsonEncode({
          "reminder_log": text,
          "remind_date": date,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          showErrorDialog(
              context, responseData['message'] ?? 'Failed to create reminder');
        }
      } else {
        handleHttpError(context, response);
      }
  ...
  Future<void> _updateReminder(
      BuildContext context, String id, String text, String date) async {
    try {
      WaitDialog.showWaitDialog(context, message: 'Updating reminder...');

      final response = await http.post(
        Uri.parse(
            '${APIHost().apiURL}/reminders_controller.php/UpdateReminder'),
        headers: HttpHeader().header,
        body: jsonEncode({
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          showErrorDialog(
              context, responseData['message'] ?? 'Failed to update reminder');
        }
      } else {
        handleHttpError(context, response);
      }
  ...
  Future<void> _deleteReminder(BuildContext context, String id) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            '${APIHost().apiURL}/reminders_controller.php/DeleteReminder'),
        headers: HttpHeader().header,
        body: jsonEncode({
          "idtbl_my_reminders": id,
          "create_by": UserCredentials().UserName,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          showErrorDialog(
              context, responseData['message'] ?? 'Failed to delete reminder');
        }
      } else {
        handleHttpError(context, response);
      }
