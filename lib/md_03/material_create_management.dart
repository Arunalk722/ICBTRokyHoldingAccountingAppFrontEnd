...
class _MaterialCreateState extends State<MaterialCreate> {
  ...
  Future<void> _loadActiveWorkList() async {
    ...
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'material_create_management.dart');
  ...
  Future<void> _loadMaterials(String? workName, String? costCategory) async {
    ...
    try {
      ...
      final response = await http.post(
        ...
        headers: HttpHeader().header,
        body:
            jsonEncode({"work_name": workName, "cost_category": costCategory}),
      );
  ...
  Future<void> _deleteMaterial(BuildContext context, int materialId) async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/material_controller.php/DeleteMaterialById';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
  ...
  Future<void> createMaterials(BuildContext context) async {
    try {
      ...
      String reqUrl =
          '${APIHost().apiURL}/material_controller.php/CreateMaterial';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
                      ...
                      children: [
                        Expanded(
                          ...
                          child: buildNumberField(
                              _txtQty, 'Qty', '1', Icons.numbers, true, 5),
                        ),
                        SizedBox(width: 10),
                        Expanded(
                            flex: 4,
  ...
  Widget _buildActiveMaterialList() {
    ...
    if (_activeMaterialListMap.isEmpty) {
      return const Center(
        child: Text('No active Billing Items found.',
        ...
        rows: _activeMaterialListMap.map<DataRow>((material) {
          return DataRow(
            cells: [
              ...
              DataCell(Text(NumberFormat('#,###.00', 'en_US')
                  .format(double.tryParse(material['amount']) ?? 0))),
              DataCell(
                IconButton(
                ...
                IconButton(
                  ...
                  onPressed: () {
                    YNDialogCon.ynDialogMessage(
                      context,
                      messageBody:
                          'Would you like to delete this material.this process cannot undo',
                      ...
                      if (value == 1) {
                        await _deleteMaterial(
                            context, material['idtbl_material_list']);
                      }
                    });
                  },
                ),
      ...
      children: [
        ...
        Autocomplete<String>(
          ...
          optionsBuilder: (TextEditingValue textEditingValue) {
            return materialList.where((word) => word
                .toLowerCase()
          ...
          onSelected: (String selection) {
            setState(() {
              ...
              _txtMaterialName.text = selection;
