...
class ApproveConstructionDialog extends StatefulWidget {
  final int requestId;
  final String refNumber;
  const ApproveConstructionDialog({
    super.key,
  ...
  @override
  ApproveConstructionDialogState createState() =>
      ApproveConstructionDialogState();
}

class ApproveConstructionDialogState extends State<ApproveConstructionDialog> {
  ...
  Future<void> _loadRequestInfo() async {
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'approve_project_payment_dialog_box.dart');
      Navigator.pop(context);
  ...
  Future<void> approveOrRejectRequest(BuildContext context, int requestId,
    ...
    try {
      String apiURL =
          '${APIHost().apiURL}/project_payment_controller.php/Approve';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
