...
Future<List<EstimationRequestSummary>> fetchEstimationRequestSummary(
    int requestId) async {
  final String reqUrl =
      '${APIHost().apiURL}/project_payment_controller.php/ViewEstimationCategory';
  PD.pd(text: reqUrl);
...
List<EstimationRequestSummary> _summarizeByCategory(
    List<EstimationRequestSummary> data) {
  final summarizedData = <EstimationRequestSummary>[];
  final categories = data.map((e) => e.costCategory).toSet();

  for (var category in categories) {
    final categoryItems =
        data.where((item) => item.costCategory == category).toList();
...
Future<void> showEstimationVsRequestDialog(
    BuildContext context, List<EstimationRequestSummary> data) {
                      ...
                      child: summarizedData.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final isNegative = item.remainingEstimateBalance
                                    .startsWith('-');
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          const SizedBox(width: 6),
                                          Text("Current Request Amount: ",
                                              style: TextStyle(
                                                  fontWeight: FontWeight.w500)),
                                          Text(
                                            NumberStyles.currencyStyle(
                                                item.currentRequestTotal),
                                            style: TextStyle(
                                                fontWeight: FontWeight.bold),
                                          ),
                                        ],
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          const SizedBox(width: 6),
                                          Text("Preview Request Amount: ",
                                              style: TextStyle(
                                                  fontWeight: FontWeight.w500)),
                                          Text(
                                            NumberStyles.currencyStyle(
                                                item.oldRequestAmount),
                                            style: TextStyle(
                                                fontWeight: FontWeight.bold),
                                          ),
                                        ],
