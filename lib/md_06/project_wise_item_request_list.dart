...
class ProjectPaymentRequestItemsReportScreenState
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            _dropdownProjects = List<String>.from((responseData['data'] ?? [])
  ...
  Future<void> _loadActiveWorkList() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            _dropdownWorkType = List<String>.from((responseData['data'] ?? [])
  ...
  Future<void> _loadActiveMaterialList(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            _dropdownMaterial = List<String>.from((responseData['data'] ?? [])
  ...
  Future<void> _dropDownToProjectLocation(String project) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            _dropdownlocations = List<String>.from((responseData['data'] ?? [])
  ...
  Future<void> fetchRequests() async {
    ...
    String apiURL =
        '${APIHost().apiURL}/project_payment_controller.php/FilteredListOfItems';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          ...
          setState(() {
            _requests = data
                .map((json) => PaymentRequestItemList.fromJson(json))
                .toList();
            _isLoading = false;
            _isLoading = false;
          });
  ...
  Widget buildTable() {
    ...
    for (var request in _requests) {
      var projectLocationKey =
          "${request.projectName} - ${request.locationName}";
      ...
      refCostTotals.update(refKey, (value) => value + costAmount,
          ifAbsent: () => costAmount);
                ...
                child: Table(
                  ...
                  children: [
                    ...
                    for (var projectLocationKey in groupedRequests.keys) ...[
                      TableRow(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Text(
                              "Project: ${projectLocationKey.split(' - ')[0]}\nLocation: ${projectLocationKey.split(' - ')[1]}",
                              style: TextStyle(
                                  color: Colors.blue[800],
                                  fontWeight: FontWeight.bold),
                            ),
                      ...
                      for (var refKey
                          in groupedRequests[projectLocationKey]!.keys) ...[
                        TableRow(
                          ...
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.all(12.0),
                              child: Text(
                                "Items: ${groupedRequests[projectLocationKey]![refKey]!.length}",
                                style: TextStyle(
                                    color: Colors.blue[800],
                                    fontWeight: FontWeight.bold),
                              ),
                        ...
                        for (var request
                            in groupedRequests[projectLocationKey]![refKey]!)
                          TableRow(
                            decoration: BoxDecoration(
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(
                                  NumberStyles.currencyStyle(
                                      '${(double.tryParse(request.requestedQuantity.toString()) ?? 0) * (double.tryParse(request.requestedAmount.toString()) ?? 0) - (double.tryParse(request.itemDis.toString()) ?? 0)}'),
                                  style: TextStyle(color: Colors.grey[800]),
                                  textAlign: TextAlign.right,
                                ),
                              ),
        ...
        Autocomplete<String>(
          ...
          onSelected: (String selection) {
            setState(() {
              ...
              _txtDropDownMaterial.text = selection;
            });
          },
          fieldViewBuilder:
            ...
            List<pw.Widget> content = [
              ...
              pw.Column(
                ...
                children: [
                  ...
                  pw.Text(
                      'Request Item Total Estimate Amount: ${NumberFormat('#,###.00', 'en_US').format(totalAmount)} LKR',
                      style: pw.TextStyle(fontSize: 12, font: ttfBold)),
                ],
              ),
              pw.SizedBox(height: 20),
              ...groupedRequests.entries
                  .map((entry) {
                    final groupReferenceNumber = entry.key;
                    final groupRequests = entry.value;

                    double groupTotalActualAmount =
                        groupRequests.fold(0.0, (sum, item) {
                      return sum +
                          (double.tryParse(item.actualAmount ?? '0') ?? 0);
                    });
                    ...
                    return [
                          ...
                          return [
                            ...
                            NumberFormat('#,###.00', 'en_US').format(
                                double.tryParse(
                                    ...
                                    0),
                            NumberFormat('#,###.00', 'en_US').format(
                                double.tryParse(request.actualAmount ?? '0') ??
                                    0),
                          ];
                        }).toList(),
                      ),
                      pw.Row(
                        ...
                        children: [
                          pw.Text(
                            'Group Total (Actual Amt): ${NumberFormat('#,###.00', 'en_US').format(groupTotalActualAmount)} LKR',
                            style: pw.TextStyle(
                                fontSize: 10,
                                fontWeight: pw.FontWeight.bold,
                                font: ttfBold),
  ...
  Future<void> exportAndPrintPaymentRequestsPdf() async {
    try {
      ...
      final pdf = await generatePaymentRequestItemListPdf(_requests);
      WaitDialog.hideDialog(context);
