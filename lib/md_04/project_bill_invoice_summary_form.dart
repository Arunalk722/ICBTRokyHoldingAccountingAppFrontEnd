...
class InvoiceSummaryDialogState extends State<InvoiceSummaryDialog> {
  ...
  Future<void> _loadInvoiceDetails() async {
    try {
      ...
      String apiURL =
          '${APIHost().apiURL}/project_payment_controller.php/GetBillSum';
      PD.pd(text: apiURL);
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            ...
            if (invoice != null) {
              _txtTotalAmount.text = NumberStyles.currencyStyle(
                  invoice['requested_amount'].toString());
              _txtVAT.text =
                  NumberStyles.currencyStyle(invoice['vat'].toString());
              _txtSSCL.text =
                  NumberStyles.currencyStyle(invoice['sscl'].toString());
              _txtAdditionalDiscount.text = NumberStyles.currencyStyle(
                  invoice['addt_discount'].toString());
              _txtItemsDiscount.text = NumberStyles.currencyStyle(
                  invoice['total_items_disc'].toString());
              _txtItemsTotal.text = NumberStyles.currencyStyle(
                  invoice['total_items_amount'].toString());
            }
          });
        } else {
...
/*  @override
      ...
      content: SingleChildScrollView(
        child: Column(
          children: [
            ...
            _buildAmountField("Items Discount", _txtItemsDiscount,
                isEnabled: false),
            _buildAmountField("Final Amount", _txtFinalAmount,
                isEnabled: false),
          ],
        ),
      ),
  ...
  bool _validateInputs() {
    if (_txtTotalAmount.text.isEmpty ||
        double.tryParse(_txtTotalAmount.text.replaceAll(',', '')) == null) {
      OneBtnDialog.oneButtonDialog(
        context,
        title: 'Error',
  ...
  Future<void> _saveInvoice(BuildContext context) async {
    ...
    try {
      String apiURL =
          '${APIHost().apiURL}/project_payment_controller.php/UpdateInvoiceSum';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
        body: jsonEncode({
          ...
          "req_ref_number": widget.refNumber,
          "requested_amount":
              double.parse(_txtTotalAmount.text.replaceAll(',', '')),
          "vat": double.tryParse(_txtVAT.text.replaceAll(',', '')) ?? 0,
          "sscl": double.tryParse(_txtSSCL.text.replaceAll(',', '')) ?? 0,
          "addt_discount": double.tryParse(
                  _txtAdditionalDiscount.text.replaceAll(',', '')) ??
              0,
