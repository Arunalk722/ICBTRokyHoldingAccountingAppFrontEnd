...
class ApprovelPendingEstimations extends StatefulWidget {
  ...
  const ApprovelPendingEstimations(
      {super.key, required this.isAuth, required this.isApprov});

  @override
  _ApprovelPendingEstimationsState createState() =>
      _ApprovelPendingEstimationsState();
}

class _ApprovelPendingEstimationsState
    extends State<ApprovelPendingEstimations> {
  ...
  Future<void> _fetchEstimationEvents() async {
    try {
      ...
      String apiUrl =
          '${APIHost().apiURL}/estimation_controller.php/ViewPendingApproveList';
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'project_pending_estimations.dart');
    } finally {
  ...
  Widget _buildSummaryCard() {
    final pendingCount = _events.where((e) => e.isAuth == 0).length;
    final totalAmount =
        _events.fold(0.0, (sum, e) => sum + e.newAmountCalc + e.previewAmount);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildAmountRow(
              "New Estimated:", event.newAmountCalc - event.previewAmount),
          _buildAmountRow("Total:", event.newAmountCalc),
          Divider(),
          Text(
              "Created by: ${event.createdBy} | ${DateFormat('MMM d, yyyy').format(event.createdDate)}",
              style: TextStyle(color: Colors.grey[600], fontSize: 12)),
          if (event.isAuth == 1 || event.isAuth == -1) ...[
            SizedBox(height: 6),
            Text(
              "${event.isAuth == -1 ? 'Authorizing Reject' : "Authorized"} by: ${event.authBy}",
              style: TextStyle(
                  color: event.isAuth == -1 ? Colors.red : Colors.green),
            ),
            Text("Comment: ${event.authCom}"),
            Text(
                "Time: ${event.authTime != null ? DateFormat('MMM d, yyyy').format(event.authTime!) : 'N/A'}"),
          ],
          if (event.isAppr == 1) ...[
            SizedBox(height: 6),
            Text(
              "${event.isAuth == -1 ? 'Approving Reject' : "Approved"} by: ${event.apprBy}",
              style: TextStyle(
                  color: event.isAuth == -1 ? Colors.red : Colors.green),
            ),
            Text("Comment: ${event.authCom}"),
            Text(
                "Time: ${event.apprTime != null ? DateFormat('MMM d, yyyy').format(event.apprTime!) : 'N/A'}"),
          ],
          SizedBox(height: 12),
          Row(
...
class ProjectLocationEstimationEvent {
  ...
  factory ProjectLocationEstimationEvent.fromJson(Map<String, dynamic> json) {
    return ProjectLocationEstimationEvent(
      ...
      authTime:
          json['auth_time'] != null ? DateTime.parse(json['auth_time']) : null,
      isAppr: json['is_appr'] ?? 0,
      apprBy: json['appr_by'] ?? 'NA',
      apprTime:
          json['appr_time'] != null ? DateTime.parse(json['appr_time']) : null,
