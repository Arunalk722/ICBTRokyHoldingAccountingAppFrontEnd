  ...
  Future<void> _dropDownToProject() async {
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'view_location_estimation.dart');
        ...
        child: SingleChildScrollView(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(
              minWidth:
                  screenWidth < minTableWidth ? minTableWidth : screenWidth,
            ),
            child: Table(
  ...
  Future<void> exportToCSV(List<Estimation> estimations) async {
    try {
      ...
      await MobileCSVExporter.export(
        ...
        headersAndRows: rows,
        fileNamePrefix:
            'location estimations $selectedProject $selectedLocation',
        onSuccess: () {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          double totalEstimate = estimationList.fold(0.0,
              (sum, item) => sum + (double.tryParse(item.estimateAmount) ?? 0));
          double totalActual = estimationList.fold(0.0,
              (sum, item) => sum + (double.tryParse(item.actualCost) ?? 0));
              ...
              data: estimationList.map((estimation) {
                return [
                  ...
                  NumberFormat('#,###.00', 'en_US')
                      .format(double.tryParse(estimation.estimateAmount) ?? 0),
                  NumberFormat('#,###.00', 'en_US')
                      .format(double.tryParse(estimation.actualCost) ?? 0),
                  NumberFormat('#,###.00', 'en_US').format(
                      double.tryParse(estimation.actualUnitAmount) ?? 0),
                ];
              }).toList(),
            ),
            pw.SizedBox(height: 20),
...
class Estimation {
  ...
  factory Estimation.fromJson(Map<String, dynamic> json) {
    return Estimation(
        ...
        idtblProjectLocationEstimationsList:
            json['idtbl_project_location_estimations_list'].toString(),
