...
class _MaterialUpdateDialogState extends State<MaterialUpdateDialog> {
  ...
  Future<void> _loadMaterialInfo() async {
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'material_editing_dialog.dart');
      Navigator.pop(context);
        ...
        CupertinoDialogAction(
          onPressed: () {
            final double updatedQty =
                double.tryParse(_txtQtyController.text) ?? 0.0;
  ...
  Future<void> changePrice(BuildContext context, int materialId, double qty,
      double amount, bool isEditAllows) async {
