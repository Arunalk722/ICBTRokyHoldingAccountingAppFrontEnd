...
class OfzExpAndTaskDesignerState extends State<OfzExpAndTaskDesigner> {
  ...
  Future<void> _createMainCategory() async {
    try {
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/InsertMainCategory';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({"main_name": _txtMainCategoryType.text}),
      );
  ...
  Future<void> _createSubCategory() async {
    try {
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/InsertSubCategory';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
  ...
  Future<void> _createdItem() async {
    try {
      String reqUrl =
          '${APIHost().apiURL}/ofz_payment_controller.php/InsertMaterialItem';
      PD.pd(text: reqUrl);
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
              ...
              if (_txtMainCategoryType.text.length > 1) {
                if (!mainCategory.contains(_txtMainCategoryType.text)) {
                  YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Confirm to create Main Category ${_txtMainCategoryType.text}',
              ...
              if (_selectedMainCategoryType == null) {
                ...
                if (!subCategories.contains(_txtSubCategoryType.text)) {
                  YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Confirm to create Sub Category ${_txtSubCategoryType.text} under Main Category Type $_selectedMainCategoryType',
              ...
              if (_selectedMainCategoryType == null) {
                ...
                if (!itemList.contains(_txtSubCategoryType.text)) {
                  YNDialogCon.ynDialogMessage(
                    context,
                    messageBody:
                        'Confirm to create Items sub Category ${_txtSubCategoryType.text} under Main Category Type $_selectedMainCategoryType',
                    messageTitle: 'Cost Category Creating',
                    icon: Icons.verified_outlined,
                    iconColor: Colors.black,
                    btnDone: 'YES',
