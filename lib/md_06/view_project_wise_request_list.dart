...
class ProjectPaymentRequestReportScreenState
  ...
  Future<void> fetchRequests() async {
    ...
    String reqUrl =
        '${APIHost().apiURL}/project_payment_controller.php/ViewAllOnRpt';
    PD.pd(text: reqUrl);
    try {
      final response = await http.post(
        Uri.parse(reqUrl),
  ...
  Widget buildTable() {
    ...
    for (var request in _requests) {
      ...
      double itemsDiscount =
          double.tryParse(request.totalItemDisc.toString()) ?? 0.0;
      ...
      projectTotals.update(groupKey, (value) => value + costAmount,
          ifAbsent: () => costAmount);
                  ...
                  children: [
                    ...
                    for (var groupKey in groupedRequests.keys) ...[
                      ...
                      for (var request in groupedRequests[groupKey]!)
                        TableRow(
                          decoration: BoxDecoration(
                            color: groupedRequests[groupKey]!.indexOf(request) %
                                        2 ==
                                    0
                                ? Colors.white
                                : Colors.grey[50],
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Text(
                                NumberStyles.currencyStyle(
                                    '${(double.tryParse(request.totalAmount) ?? 0) + (double.tryParse(request.vat ?? '0') ?? 0) + (double.tryParse(request.sscl ?? '0') ?? 0) - (double.tryParse(request.addtDiscount ?? '0') ?? 0) - (double.tryParse(request.totalItemDisc ?? '0') ?? 0)}'),
                                style: TextStyle(color: Colors.grey[800]),
                                textAlign: TextAlign.right,
                              ),
                            ),
