...
class ViewMaterialListPage extends StatefulWidget {
  ...
  @override
  ViewMaterialListPageState createState() => ViewMaterialListPageState();
}

class ViewMaterialListPageState extends State<ViewMaterialListPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            workType = List<String>.from((responseData['data'] ?? [])
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'view_material_list.dart');
  ...
  Future<void> _dropDownToCategory(String workName) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(reqUrl),
        headers: HttpHeader().header,
        body: jsonEncode({"work_name": workName}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 200) {
          setState(() {
            category = List<String>.from((responseData['data'] ?? [])
  ...
  Future<void> _loadworkTypeLocationEstimationList() async {
    try {
      ...
      final response = await http.post(
        ...
        headers: HttpHeader().header,
        body: jsonEncode(
            {"work_name": selectedWorkType, "cost_category": selectedCategory}),
      );
                ...
                children: [
                  ...
                  for (var workType in groupedMaterials.keys)
                    for (var category in groupedMaterials[workType]!.keys) ...[
                      ...
                      for (var material
                          in groupedMaterials[workType]![category]!)
                        TableRow(
                          decoration: BoxDecoration(
                          ...
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.all(12.0),
                              child: Text(
                                '${NumberStyles.qtyStyle(material.qty)} ${material.uom}',
                                style: TextStyle(
                                    color: Colors.grey[800], fontSize: 14),
  ...
  Future<void> exportToCSV(
      BuildContext context, List<MaterialListItem> materials) async {
    try {
  ...
  Future<pw.Document> generateEstimationPdf(
    List<MaterialListItem> estimationList,
    String projectName,
    String locationName,
  ) async {
              ...
              data: estimationList.map((estimation) {
                return [
                  ...
                  NumberFormat('#,###.00', 'en_US')
                      .format(double.tryParse(estimation.amount))
                ];
              }).toList(),
            ),
            pw.SizedBox(height: 20),
...
class MaterialListItem {
  ...
  factory MaterialListItem.fromJson(Map<String, dynamic> json) {
    return MaterialListItem(
      ...
      idtblMaterialCostCategory:
          json['idtbl_material_cost_category'].toString(),
