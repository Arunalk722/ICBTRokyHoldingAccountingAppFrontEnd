...
class AuthorizationConstructionDialog extends StatefulWidget {
  ...
  @override
  AuthorizationConstructionDialogState createState() =>
      AuthorizationConstructionDialogState();
}

class AuthorizationConstructionDialogState
    extends State<AuthorizationConstructionDialog> {
  late bool isToCEO;
  ...
  Future<void> _loadRequestInfo() async {
    try {
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'auth_project_payment_dialog_box.dart');
      Navigator.pop(context);
  ...
  Future<void> approveOrRejectRequest(BuildContext context, int requestId,
    ...
    try {
      String apiURL =
          '${APIHost().apiURL}/project_payment_controller.php/Authorize';
      PD.pd(text: apiURL);
      final response = await http.post(
        Uri.parse(apiURL),
        headers: HttpHeader().header,
