...
class PaymentRequestData {
  ...
  factory PaymentRequestData.fromJson(Map<String, dynamic> json) {
    return PaymentRequestData(
      ...
      totalSpentAmountPreviewAll:
          _parseDouble(json['totalSpentAmountPreviewAll']),
...
class EstimationDetailsDialog extends StatefulWidget {
  ...
  @override
  State<EstimationDetailsDialog> createState() =>
      _EstimationDetailsDialogState();
}
...
class _EstimationDetailsDialogState extends State<EstimationDetailsDialog> {
  ...
  Future<void> _fetchEstimationData(String projectName, String location) async {
    ...
    try {
      String apiURL =
          '${APIHost().apiURL}/report_controller.php/RequestWiseItems';
      PD.pd(text: apiURL);
      ...
      ExceptionLogger.logToError(
          ...
          logFile: 'estimation_category_wise_consume_list.dart');
      setState(() {
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_isLoading && _errorMessage.isEmpty)
                _buildGrandTotal(
                  totalEstimated:
                      _groupedItems.values.first.first.totalProjectEstimated,
                  totalSpent: _groupedItems
                      .values.first.first.totalSpentAmountPreviewAll,
                  isMobile: isMobile,
                ),
            ],
          ),
  ...
  Widget _buildProgressBar(PaymentRequestData item, bool isMobile) {
    final percentage = item.categoryBudget > 0
        ? ((item.currentRequestAmount + item.previousRequestsTotal) /
                item.categoryBudget)
            .clamp(0.0, 1.0)
        : 0.0;
