...
class ViewUserRequestListState extends State<ViewUserRequestList> {
  ...
  Future<List<PaymentRequest>> fetchRequests() async {
    String apiURL =
        '${APIHost().apiURL}/project_payment_controller.php/ViewUserRequest';
    PD.pd(text: apiURL);
  ...
  Future<List<PaymentRequest>> fetchRequestsOfzPayment() async {
    String apiURL =
        '${APIHost().apiURL}/ofz_payment_controller.php/ViewUserRequest';
    PD.pd(text: apiURL);
    final response = await http.post(
      Uri.parse(apiURL),
      headers: HttpHeader().header,
            ...
            child: ExpansionTile(
              ...
              title: Row(
                children: [
                  ...
                  StatusIcon(
                      statusId: int.tryParse(request.statusId.toString()) ?? 0),
                ],
              ),
              children: [
                Padding(
                            ...
                            children: [
                              ...
                              FilledButton.icon(
                                ...
                                onPressed: () => Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => request.reqType ==
                                            'OFZ'
                                        ? ViewOfzRequestList(
                                            requestId: request.id.toString(),
                                            isNotApprove: true,
  ...
  Future<void> _postRequest(String reqId) async {
    try {
      String idString =
          RequestNumber.formatNumber(val: int.tryParse(reqId) ?? 0);
